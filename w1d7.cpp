...
void DIJKSTRA(int sv,int n){
	...
	vector<int> vis(N,0),distance(N,inf);
	...
	while(pq.size()>0){
		...
		for(auto child:adjlist[cv]){
			int childv = child.f;
			int childd = child.s;
			if(distance[cv]+childd < distance[childv]){
				distance[childv] = distance[cv] + childd;
				debug(mp(distance[cv],childd))
				debug(distance[childv])
				pq.insert({distance[childv],childv});
			}
		}
	}

	fr(i,1,n+1){
		if(distance[i]==inf) cout<<"I ";
